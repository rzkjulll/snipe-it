...
class Helper
{
    ...
    */
    ...
    public static function AutoGenerateAssetTags($location, $purchase_date, $model_name, $sequence, $endfix){
        // Validate Location Name
        if(strlen($location) > 5){
            $location = strtoupper(substr($location, 0, 5));
        }
        
        // trim date to only show 2 digit years and month
        $pd = strtotime($purchase_date);
        $date = date("ym",$pd);

        // trim category model for category name that has more than 2 string
        // validate model name
        if(strlen($model_name) < 2) {
            $model_name = "O".$model_name;
        } else if(strlen($model_name) > 2) {
            $model_name = strtoupper(substr($model_name, 0, 2));
        }

        // Sequence 
        $sequence = ($sequence < 10) ? "0".$sequence : $sequence;

        $assetTags = $location.$date.$model_name.$sequence.$endfix;

        return $assetTags;
    }

    public static function getEndfixName($id){
        //HQ= 1 WH= 2 WH-Buffered= 3-5 FM= 6 SH= 7 AL= 8
        $name = "Unknown";
        if($id == 1){
            $name = "HQ";
        }
