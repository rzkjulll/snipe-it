...
class AssetsController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if(!isset($user->groups[0])){
            session()->flash('error', 'Missing Account Permision Group, please contact your admin.');
            return view('hardware/index');
        }

        if($user->groups[0]->name != "Super Admin" && !isset($user->location_id)){
            session()->flash('error', 'Missing Account Permision Group, please contact your admin.');
            return view('hardware/index');
        }
        ...
        return view('hardware/index')->with('company', $company)->with('getAssetsUrl', $getAssetsUrl);
    }
    ...
    public function store(ImageUploadRequest $request)
    {
        ...
        $location_name = Location::find(request('rtd_location_id', null))->name;

        // Get Category Name
        $assetmodel = AssetModel::with('category')->get()->find($request->input('model_id'));
        $category_name = $assetmodel->category->name;

        // Check Sequence Value
        $sequence = intval($request->input('sequence'));
        if($sequence == 0){ // 0 means use auto increment
            // Get Last Sequence
            $_sequence = Asset::select('sequence')->where([
                ['model_id',$request->input('model_id')],
                ['rtd_location_id',$request->input('rtd_location_id')],
                ['endfix',$request->input('endfix')]
            ])->orderByDesc('sequence')->first();

            $sequence = intval($_sequence->sequence) > 0 ? intval($_sequence->sequence)+1 : 1;
        }

        // Generate and Validate Asset Tags
        $asset_tags = Helper::AutoGenerateAssetTags($location_name, request('purchase_date', null), $category_name, $sequence, $endfix = $request->input('endfix'));
        /*if(strlen($asset_tags) != 14){
            return redirect()->back()->withInput()->withErrors("Invalid Asset Tags Format, make sure all the value is correct");
        }*/
        ...
        $asset->serial                  = request('serials', null);
        ...
        $asset->mac_address             = request('mac_address', null);
        $asset->sequence                = $sequence;
        $asset->endfix                  = $request->input('endfix');
    ...
    public function show($assetId = null)
    {
        ...
        if (isset($asset)) {
            ...
            $asset->endfix = ($asset->endfix) ? Helper::getEndfixName($asset->endfix) : null;
    ...
    public function update(ImageUploadRequest $request, $assetId = null)
    {
        ...
        // Get Warehouse Location Name
        $location_name = Location::find($request->input('rtd_location_id'))->name;

        // Get Category Name
        $assetmodel = AssetModel::with('category')->get()->find($request->get('model_id'));
        $category_name = $assetmodel->category->name;
        ...
        // Generate and Validate Asset Tags
        $asset_tags = Helper::AutoGenerateAssetTags($location_name, request('purchase_date'), $category_name, $sequence, $request->input('endfix'));
        /*if(strlen($asset_tags) != 14){
            return redirect()->back()->withInput()->withErrors("Invalid Asset Tags Format, make sure all the value is correct");
        }*/
